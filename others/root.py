...
from numpy import array as aray
from numpy import copy as arr
from numpy import argmax as ar
...
class Root:
    ...
    def levy_flight(self, epoch=None, position=None, g_best_position=None, step=0.001, case=0):
        ...
        sigma_muy = power(
            gamma(1 + beta) * sin(pi * beta / 2) / (gamma((1 + beta) / 2) * beta * power(2, (beta - 1) / 2)), 1 / beta)
    ...
    def levy_flight_2(self, position=None, g_best_position=None):
        alpha = 0.01
        xichma_v = 1
        xichma_u = ((gamma(1 + 1.5) * sin(pi * 1.5 / 2)) / (gamma((1 + 1.5) / 2) * 1.5 * 2 ** ((1.5 - 1) / 2))) ** (
                1.0 / 1.5)
...
class model:
    def __init__(self, m1, m2, ft, xx, yy, zz):
        self.m1, self.m2 = m1, m2
        self.ft, self.xx = ft, xx
        self.yy, self.zz = yy, zz

    def predict_classes(self):
        c1, c2, ft = self.m1, self.m2, self.ft
        pred = ft.reshape(1, -1, ft.shape[0])
        c1.predict_classes(pred).flatten()
        c2.predict_classes(pred).flatten()
        return [c1, c2, self.yy[self.zz]]
